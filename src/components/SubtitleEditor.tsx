...
import { BatchTimeShiftModal } from "./BatchTimeShiftModal";
...
export const SubtitleEditor: React.FC<SubtitleEditorProps> = ({
  ...
  const filteredCues = cues.filter(
    (cue) =>
      cue.hebrewText.toLowerCase().includes(searchTerm.toLowerCase()) ||
      cue.originalText.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleApplyBatchShift = (shiftSeconds: number, targetScope: "all" | "selected") => {
    if (targetScope === "all" || selectedCueIds.length === 0) {
      onShiftAllTimings(shiftSeconds);
      setFeedbackMessage({
        type: "success",
        text: `כל ${cues.length} הכתוביות הוזזו ב-${shiftSeconds > 0 ? "+" : ""}${shiftSeconds * 1000}ms!`,
      });
    } else {
      ...
      setFeedbackMessage({
        type: "success",
        text: `הוזזו בהצלחה ${selectedCueIds.length} כתוביות נבחרות ב-${shiftSeconds > 0 ? "+" : ""}${shiftSeconds * 1000}ms!`,
      });
    }
  };
  ...
  // Merge individual cue with the next consecutive cue
  const handleMergeWithNext = (index: number) => {
    if (index >= cues.length - 1) return;
    const cueA = cues[index];
    const cueB = cues[index + 1];
    ...
    setFeedbackMessage({
      type: "success",
      text: `כתובית #${index + 1} אוחדה בהצלחה עם כתובית #${index + 2}!`,
    });
  };
  ...
  return (
    <div className="bg-[#141414] border border-[#222222] rounded-xl p-4 lg:p-5 flex flex-col h-full shadow-xl relative" id="subtitle-editor">
      {/* Top Header */}
      <div className="flex flex-col gap-3 pb-3 border-b border-[#222222]">
        <div className="flex items-center justify-between flex-wrap gap-2">
          {/* Title, Count & Auto-Save Badge */}
          <div className="flex items-center flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-[#1a1a1a] text-blue-400 text-xs px-2.5 py-0.5 rounded-full font-mono border border-[#333333]">
                {cues.length} שורות
              </span>
            </div>

            {/* Overlap Warning Badge in Header if detected */}
            {totalOverlapCount > 0 && (
              <div
                className="flex items-center gap-1.5 bg-rose-950/80 border border-rose-500/60 text-rose-300 px-2.5 py-0.5 rounded-md text-[11px] font-bold animate-pulse shadow-sm"
                title={`${totalOverlapCount} כתוביות בעלות חפיפת זמנים (מודגשות במסגרת אדומה)`}
              >
                <AlertCircle className="w-3.5 h-3.5 text-rose-400" />
                <span>זוהו {totalOverlapCount} חפיפות זמנים!</span>
              </div>
            )}

            {/* Auto-save status indicator */}
            <div
              className="flex items-center gap-1.5 bg-[#111827] border border-blue-900/40 text-blue-300 px-2 py-0.5 rounded-md text-[11px] font-mono shadow-inner"
              title={
                lastAutoSavedAt
            ...
            >
              ...
              <span>
                {lastAutoSavedAt ? `נשמר ${lastAutoSavedAt}` : "שמירה אוטומטית פעילה"}
              </span>
            </div>
          </div>

          {/* Right Controls: Manual Save, Tone Preference & Target Language */}
          <div className="flex items-center gap-2">
            {/* Quick Save Now Button */}
            <button
              onClick={handleSaveClick}
              className="flex items-center gap-1 px-2.5 py-1.5 bg-[#1c1c1c] hover:bg-[#262626] text-gray-200 hover:text-white border border-[#333333] hover:border-emerald-500/50 rounded-lg text-xs font-semibold transition cursor-pointer"
              title="שמור את השינויים מיד ב-localStorage"
            >
              <Save className="w-3.5 h-3.5 text-emerald-400" />
              <span>שמור עכשיו</span>
            </button>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[#1c1c1c] hover:bg-[#262626] text-white border border-[#333333] hover:border-purple-500/50 rounded-lg text-xs font-semibold transition shadow-sm cursor-pointer"
                title="בחר סגנון ותרגום (Tone Preference): רשמי, יומיומי או מילולי"
              >
                <SlidersHorizontal className="w-3.5 h-3.5 text-purple-400" />
                <span className="text-gray-400 text-[11px]">סגנון:</span>
              ...
              {showToneMenu && (
                <div
                  className="absolute left-0 md:left-auto md:right-0 mt-1.5 w-60 bg-[#161616] border border-[#2e2e2e] rounded-xl shadow-2xl p-1.5 z-50 animate-in fade-in slide-in-from-top-2"
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1c1c1c] hover:bg-[#262626] text-white border border-[#333333] hover:border-blue-500/50 rounded-lg text-xs font-semibold transition shadow-sm cursor-pointer"
              ...
              {showLanguageMenu && (
                <div
                  className="absolute left-0 md:left-auto md:right-0 mt-1.5 w-56 bg-[#161616] border border-[#2e2e2e] rounded-xl shadow-2xl p-1.5 z-50 animate-in fade-in slide-in-from-top-2 max-h-72 overflow-y-auto custom-scrollbar"
        ...
        <div className="flex flex-col gap-2">
          ...
          {/* Secondary Utilities: Find & Replace, Batch Time Shift, OCR Cleaner, Merge Short Cues */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-[#1f1f1f]">
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-[#1c1c1c] hover:bg-[#282828] text-purple-300 hover:text-purple-200 border border-purple-500/30 hover:border-purple-500/50 rounded-md text-xs font-medium transition disabled:opacity-40 disabled:pointer-events-none cursor-pointer"
              ...
              {/* Batch Time Shift Button */}
              <button
                id="batch-time-shift-btn"
                onClick={() => setBatchShiftModalOpen(true)}
                disabled={cues.length === 0}
                className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-[#1c1c1c] hover:bg-[#282828] text-cyan-300 hover:text-cyan-200 border border-cyan-500/30 hover:border-cyan-500/50 rounded-md text-xs font-medium transition disabled:opacity-40 disabled:pointer-events-none cursor-pointer"
                title="הזזת תזמונים מדויקת במילישניות עבור כל הכתוביות או נבחרות"
              >
                <SlidersHorizontal className="w-3.5 h-3.5 text-cyan-400" />
                <span>הזזת תזמון (ms)</span>
                {selectedCueIds.length > 0 && (
                  <span className="bg-cyan-950 text-cyan-300 text-[10px] px-1.5 py-0.2 rounded-full border border-cyan-500/40">
            ...
            {/* OCR Cleaner, Merge Short Cues, Auto-Format, Quick Search */}
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              >
                ...
                <span>עיצוב אוטומטי (Auto-Format)</span>
              </button>
              ...
              <div className="relative flex-1 min-w-[120px]">
                ...
                <input
                  ...
                  className="w-full bg-[#0d0d0d] border border-[#262626] rounded-md pr-8 pl-2 py-1 text-xs text-gray-200 placeholder-gray-500 focus:outline-none focus:border-blue-500 transition"
                />
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    {/* Merge with next cue */}
                    {idx < filteredCues.length - 1 && (
                      <button
                        onClick={() => handleMergeWithNext(idx)}
                        className="p-1 rounded bg-[#1e1e1e] hover:bg-blue-600/80 text-gray-400 hover:text-white transition cursor-pointer"
                        title="אחד עם הכתובית הבאה ברצף"
                      >
      ...
      <BatchTimeShiftModal
        ...
        onApplyShift={handleApplyBatchShift}
      />
